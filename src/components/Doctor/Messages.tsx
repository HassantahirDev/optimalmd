...
const Messages: React.FC<MessagesProps> = ({ doctorId: propDoctorId }) => {
  ...
  const initializeSocket = () => {
    console.log('Initializing socket with doctorId:', doctorId);
  ...
  const loadConversations = async () => {
    try {
      ...
      const response = await api.get(`/messages/conversations?userId=${doctorId}&userType=doctor`);
  ...
  const loadMessages = async (participantId: string) => {
    try {
      ...
      const unreadMessages = messages.filter((msg: Message) => 
        msg.receiverId === doctorId && !msg.isRead
      );
      ...
      <Card className="bg-gray-800 border-gray-700">
        <CardContent className="p-6">
          <div className="text-center text-gray-400">
            ...
            <p>Doctor ID is required to access messages</p>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[625px]">
          ...
          <Card className="bg-gray-800 border-gray-700 flex flex-col">
            <CardHeader className="flex-shrink-0">
              ...
              <p className="text-gray-400 text-sm">Showing all patients for testing</p>
            </CardHeader>
