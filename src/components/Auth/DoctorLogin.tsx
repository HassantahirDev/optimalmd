  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        const { accessToken, doctor, userType } = response.data.data;
        
        // Store auth data in localStorage
        localStorage.setItem("authToken", accessToken);
        localStorage.setItem("userType", userType);
        localStorage.setItem("userId", doctor.id);
        localStorage.setItem("name", `${doctor.firstName} ${doctor.lastName}`);
        localStorage.setItem("profilePicture", doctor.profilePicture || "");
        
        toast.success("Login successful! Redirecting to doctor dashboard...");
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400">Access the doctor dashboard</p>
        </div>
        ...
        <div className="mt-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700">
          <div className="flex items-start gap-3">
            <Stethoscope className="h-5 w-5 text-yellow-500 mt-0.5 flex-shrink-0" />
            <div>
              <h3 className="text-yellow-500 font-medium text-sm mb-1">Medical Access Notice</h3>
              <p className="text-gray-400 text-xs">
                This is a secure medical area. All login attempts are logged and monitored for patient safety.
              </p>
            </div>
          </div>
        </div>
